...
/// On macOS, this is `~/Library/Application Support/Ompzed`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/ompzed`.
/// On Windows, this is `%LOCALAPPDATA%\Ompzed`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/ompzed`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/ompzed`.
/// On Windows, this is `%APPDATA%\Ompzed`.
